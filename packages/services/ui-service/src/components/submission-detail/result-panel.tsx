  ...
  return (
    <Panel>
      <PanelHeader>
        ...
        <span className="font-mono text-xs text-muted-foreground">tag/output/default</span>
      </PanelHeader>
      {!job || !readout.present ? (
          ...
          <PanelBody className="p-2">
            ...
            {readout.headline ? (
              <div className="mt-1 flex items-center justify-between gap-4 rounded-lg bg-muted px-3 py-2.5">
                <span className="text-sm font-semibold">{readout.headline.label}</span>
  ...
  return (
    <Panel>
      <PanelHeader>
        ...
        <span className="font-mono text-xs text-muted-foreground">{job.outputs.length}</span>
      </PanelHeader>
      <PanelBody className="space-y-3">
        {job.outputs.map((output) => (
          <div key={output.id} className="rounded-lg border border-border p-4">
            <p className="font-mono text-xs text-muted-foreground">{output.reference}</p>
