...
describe("row", () => {
  ...
  it("refuses a nested object, and says which key", () => {
    ...
    expect(() => row({ results: { q1: 1 } }, "reduce()")).toThrow(/results is an object/);
  });
  ...
  it("names every offending key at once", () => {
    expect(() => row({ a: {}, b: [], c: 1 }, "reduce()")).toThrow(/a is an object, b is a list/);
  });
  ...
  it("refuses a score that is not finite", () => {
    ...
    expect(() => row({ score: Number.POSITIVE_INFINITY }, "reduce()")).toThrow(/score is Infinity/);
  ...
  it("names where the trouble was", () => {
    expect(() => row({ a: {} }, "evaluate() on case 3/40")).toThrow(/evaluate\(\) on case 3\/40/);
  });
});
