...
import { GITHUB_REF_SELECT_KIND, REPOSITORY_CARD } from "./constants";
import {
  ensureParticipantRepository,
  githubOrg,
  maxArchiveBytes,
  octokit,
  participantRepositoryName,
} from "./github";
import { listLatestRefsForUser, resolveSelectedRef } from "./refs";
import repositoryCard from "./repository-card";
import { content } from "./surfaces";
...
export default {
  ...
  runner: {
    setup: async ({ job }, next) => {
      ...
      // reference. Base64 in a database row inflated it by a third, could not be
      // streamed, and put every submission in every backup forever.
      ...
      // Reaching the fallback means no runner is lined up behind us.
      return (await next?.({ job })) ?? { status: "prepared" };
    },
